...
export default (props: CardProps & React.RefAttributes<HTMLDivElement> & { updatesHeap: any; layoutUpdate: number; settings?: boolean }) => {
    const totalMemoryGb = props?.updatesHeap?.hardware?.memoryBanks?.reduce((prev: any, curr: any) => prev + curr, 0)
    ...
    return <div {...props} style={{ position: 'relative' }}>
        ...
        <Card>
            ...
            <List>
                ...
                <ListItem >
                    <span>Memory</span>
                    <span>{props?.updatesHeap?.hardware?.memoryBanks?.map((v: any) => `${v}GB`).join(' + ')} = {totalMemoryGb}GB</span>
                </ListItem>
