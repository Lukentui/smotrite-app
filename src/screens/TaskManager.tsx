import { Badge, Card, CardProps, Flex, Text, Metric, List, ListItem, Switch, TextInput, Button } from "@tremor/react"
...
const iconPlaceHolder = 'data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP89B8AAukB8/71MdcAAAAASUVORK5CYII='

const sortDirections = {
  'desc': <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
  ...
  'asc': <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
  ...
  'unknown': <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
    <path strokeLinecap="round" strokeLinejoin="round" d="M3 7.5L7.5 3m0 0L12 7.5M7.5 3v13.5m13.5 0L16.5 21m0 0L12 16.5m4.5 4.5V7.5" />
  </svg>
}

const deleteIcon = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
...
const openIcon = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
...
const filterIcon = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
  <path fill-rule="evenodd" d="M2.628 1.601C5.028 1.206 7.49 1 10 1s4.973.206 7.372.601a.75.75 0 01.628.74v2.288a2.25 2.25 0 01-.659 1.59l-4.682 4.683a2.25 2.25 0 00-.659 1.59v3.037c0 .684-.31 1.33-.844 1.757l-1.937 1.55A.75.75 0 018 18.25v-5.757a2.25 2.25 0 00-.659-1.591L2.659 6.22A2.25 2.25 0 012 4.629V2.34a.75.75 0 01.628-.74z" clip-rule="evenodd" />
</svg>

const searchIcon = <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-4 h-4">
...
const viseVersaDirection = (direction: 'asc' | 'desc') => direction === 'asc' ? 'desc' : 'asc'
export default () => {
  const [a,] = useState(1)
  ...
  const [sort, setSort] = useState<{ column: string, direction: 'asc' | 'desc' }>({ column: 'cpuPercent', direction: 'asc' })
  ...
  useEffect(() => {
    ...
    (window as any).api.on("kill-process-error", (data: any) => {
      ...
      // setTimeout(() => setAlternativeTooltipContent({
      //   ...alternativeTooltipContent,
  ...
  const processedTasks = useMemo(() => {
    // if(!liveUpdates) {
    //   return lastProcessedTasks.current
    // }

    const res = (liveUpdates ? tasks : lastProcessedTasks.current).filter((_, i) => i < 25000).sort((a, b) => {
  ...
  const handleSort = (column: string) => setSort({ column, direction: viseVersaDirection(sort.direction) })
  ...
  return <Grid numItems={1} numItemsSm={1} numItemsLg={1} className="gap-4" style={{
    padding: '30px',
    paddingTop: '15px',
    paddingBottom: 0,
  }}>
    <Card className="task-manager__primary-filters-card">
      {/* <div className="search-input">
        <TextInput icon={SearchIcon} placeholder="Search..." style={{ boxShadow: 'none' }} />
      </div> */}

      <div className="suspend-switch flex items-center justify-end space-x-3">
    ...
    <Card className="task-manager__primary-card" style={{ paddingBottom: 0 }}>
      <div className="task-manager__header">
        <div className="filters">
          <span className={`dd-button ${liveUpdates ? '' : 'blinking'}`} onClick={() => setLiveUpdates((v) => !v)}>{liveUpdates ? 'Pause' : 'Resume'}</span>
        </div>
        ...
        >
          <span className="pid-sort-lg">{sort.column === 'pid' && sortDirections[sort.direction]}</span>
          PID
          <span className="pid-sort-sm">{sort.column === 'pid' && sortDirections[sort.direction]}</span>
        </div>
...
{/* 
        ...
        >{sort.column === 'cpuPercent' && sortDirections[sort.direction]} CPU</div>

        <div
          className="mem clickable"
          onClick={() => handleSort('memBytes')}
        >{sort.column === 'memBytes' && sortDirections[sort.direction]} RAM</div>
      </div>
      ...
      {/* <div className="task-manager__list"> */}
        ...
        {processedTasks.map((item) => (
          <div key={item.pid} className="task-manager__task">
            <Tooltip
              isOpen={alternativeTooltipContent[`${item.pid}:${item.name}`] ? true : undefined}
              anchorSelect={`#kill-${item.pid}`}
              content={alternativeTooltipContent[`${item.pid}:${item.name}`] ?? "Kill the process"}
              place="left"
            ...
            <div className="icon-picture">
              {
                item.icon && <LazyLoadImage src={`local-fs://` + item.icon} height={24} width={24} />
              }

              {!item.icon && <img src={iconPlaceHolder} style={{ borderRadius: 8 }} height={24} width={24} />}
            </div>
            ...
            <div>
              <div className="process-name-and-actions">
                ...
                <div className="actions">
                  <div className="action-button" id={`open-folder-${item.pid}`} onClick={() => handleOpenFolder(item)}>
                    {openIcon}
                  </div>
                  <div className="action-button" id={`kill-${item.pid}`} onClick={() => handleKill(item)}>
                    {deleteIcon}
                  </div>
                </div>
              </div>
...
// <div className="suspend-switch item flex items-center justify-between space-x-3">
